...
var creatorList = ['@alfian','@RenBotz'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'RenBotz x AlfianGanz'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'RenBotz x AlfianGanz') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'RenBotz x AlfianGanz') return res.json(loghandler.invalidKey)
...
router.get('/fbdown', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'RenBotz x AlfianGanz') return res.json(loghandler.invalidKey)
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "RenBotz x AlfianGanz"})
